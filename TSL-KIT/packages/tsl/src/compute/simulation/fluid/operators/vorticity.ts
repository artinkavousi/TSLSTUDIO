...
interface VorticityParams {
  dimensions: GridDimensions
  velocity: ShaderNode
  vorticity: ShaderNode
  config: FluidUniforms
}

export const vorticityMagnitudeShader = ({ dimensions, velocity, vorticity, config }: VorticityParams) =>
  Fn(() => {
    const shader = new ComputeShader('fluidVorticity')
    shader.workgroupSize = [4, 4, 4]

    shader.setBindings([
      ShaderNodeUniform(config),
      { resource: 'storage', node: velocity },
      { resource: 'storage', node: vorticity },
    ])
...
interface VorticityConfinementParams {
  dimensions: GridDimensions
  velocity: ShaderNode
  vorticity: ShaderNode
  config: FluidUniforms
...
export const vorticityConfinementShader = ({ dimensions, velocity, vorticity, config }: VorticityConfinementParams) =>
  Fn(() => {
    const shader = new ComputeShader('fluidVorticityConfinement')
    shader.workgroupSize = [4, 4, 4]
