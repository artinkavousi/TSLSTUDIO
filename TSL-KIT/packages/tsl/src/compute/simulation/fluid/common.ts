export const FLUID_COMMON_SOURCE = /* wgsl */ `
...
fn sinplex(p: vec3f) -> f32 {
    return .5 + .5 * (sin(p.x) * sin(p.y * 1.5) * sin(p.z * 2.5));
}

fn add_smoke(t: vec4f, p: vec4f) -> vec4f {
    var w = p.a + t.a;
    return select(vec4(mix(p.rgb, t.rgb, t.a / w), w), t, w == 0);
}

fn simplex(p: vec3f) -> f32 {
    ...
    var b = x - i + 1. / 3;
    var c = x - .5;
    var w = max(.5 - vec4f(dot(x, x), dot(a, a), dot(b, b), dot(c, c)), vec4f(0));
    return .5 + 20 * dot(vec4f(
        dot(1-2*D(s), x),
        dot(1-2*D(s + j), a),
        dot(1-2*D(s + i), b),
        dot(1-2*D(s + 1), c)
...
fn clamp_to_edge(id: vec3i) -> u32 {
    return to_index(vec3u(clamp(id, vec3i(0), vec3i(vec3u(u.ux, u.uy, u.uz)) - 1)));
}

fn trilerp1(texture: ptr<storage, array<f32>, read>, pos: vec3f) -> f32 {
    let base = vec3i(pos + 0.5) - 1;
...
fn trilerp4(texture: ptr<storage, array<vec4f>, read>, pos: vec3f) -> vec4f {
    let base = vec3i(pos + 0.5) - 1;
    let frac = fract(pos + 0.5);
    return mix(
        mix(
