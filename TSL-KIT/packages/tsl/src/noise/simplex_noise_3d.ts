...
/**
 * 3D simplex noise implementation adapted for TSL.
 */
export const simplexNoise3d = Fn<readonly [Vec3Node]>(
  ([input]) => {
    ...
    const x2 = vec3(x0.sub(i2).add(C.xxx.mul(2.0))).toVar()
    const x3 = vec3(x0.sub(1.0).add(C.xxx.mul(3.0))).toVar()
    ...
    const j = vec4(p.sub(49.0 * floor(p.mul(ns.z).mul(ns.z)))).toVar()
    const x_ = vec4(floor(j.mul(ns.z))).toVar()
    const y_ = vec4(floor(j.sub(7.0 * x_))).toVar()
    ...
    return 42.0 * dot(m.mul(m), vec4(dot(p0, x0), dot(p1, x1), dot(p2, x2), dot(p3, x3)))
  },
).setLayout({
  name: 'simplexNoise3d',
  type: 'float',
