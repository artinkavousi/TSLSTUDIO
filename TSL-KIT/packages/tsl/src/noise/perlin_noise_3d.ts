...
import { fade, mod289, permute, taylorInvSqrt } from './common'; 

/**
 * 3D Perlin noise implementation.
 */
export const perlinNoise3d = Fn<readonly [Vec3Node]>(
  ([input]) => {
    ...
    const nZ = vec4(mix(vec4(n000, n100, n010, n110), vec4(n001, n101, n011, n111), fadeXYZ.z)).toVar()
    const nYZ = vec2(mix(nZ.xy, nZ.zw, fadeXYZ.y)).toVar()

    return float(2.2).mul(mix(nYZ.x, nYZ.y, fadeXYZ.x))
  },
).setLayout({
  name: 'perlinNoise3d',
  type: 'float',
