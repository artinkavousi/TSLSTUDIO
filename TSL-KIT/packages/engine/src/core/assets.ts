...
export interface EXRTextureOptions {
  onProgress?: (event: ProgressEvent<EventTarget>) => void
  signal?: AbortSignal
  type?: TextureDataType
  cacheKey?: string
...
export interface KTX2TextureOptions extends TextureLoadOptions {
...
const ensureColorSpace = (colorSpace?: string) => colorSpace ?? DEFAULT_COLOR_SPACE

const cacheKey = (key: string, custom?: string) => (custom ? `${key}:${custom}` : key)
