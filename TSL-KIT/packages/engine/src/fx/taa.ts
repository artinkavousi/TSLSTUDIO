...
import { float, mix, texture, uniform, uv, vec4 } from 'three/tsl'

import type { FramePass, FramegraphExecutionContext, FramegraphSetupContext } from 
...
export type TemporalAAPreset = 'low' | 'medium' | 'high'
...
export type TemporalAAOptions = {
  ...
  customSequence?: HaltonPair[]
  priorityOffset?: number
  resolvePriority?: number
}

const PRESET_OPTIONS: Record<TemporalAAPreset, TemporalAAOptions> = {
  low: {
    sampleCount: 4,
    response: 0.8,
    clampDiff: 0.02,
...
export function getTemporalAAPreset(name: TemporalAAPreset): TemporalAAOptions {
  return PRESET_OPTIONS[name]
}
...
function ensureResolver(state: TemporalAAState, renderer: import('three/webgpu').WebGPURenderer, clampDiff: number) {
  if (!state.resolver) {
...
export function createTemporalAAPasses(options: TemporalAAOptions = {}): TemporalAAPasses {
  ...
  const haltonSequence = options.customSequence?.length ? options.customSequence : halton23Sequence(sampleCount)
  const jitterSpread = options.jitterSpread ?? 0.75
  const jitterOffset = {
    x: options.jitterOffset?.x ?? 0,
    y: options.jitterOffset?.y ?? 0,
  }
  ...
  const jitterPass: FramePass = {
    ...
    exec: (context: FramegraphExecutionContext) => {
      ...
      const jitter = new Vector2(
        (sample[0] - 0.5 + jitterOffset.x) * jitterSpread,
        (sample[1] - 0.5 + jitterOffset.y) * jitterSpread,
      )
